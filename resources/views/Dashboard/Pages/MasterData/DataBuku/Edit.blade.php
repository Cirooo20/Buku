...
@section('konten')
    <div class="container mx-auto px-4 py-8">
        <div class="mb-8">
            <h1 class="text-3xl font-bold mb-2 text-white">Edit Data Buku</h1>
            <p class="text-gray-200">MasterData > Data Buku > Edit</p>
        </div>
            ...
            <form method="post" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    <div class="mb-4">
                        ...
                        <textarea id="sinopsis" name="sinopsis" rows="3" placeholder="Masukkan Sinopsis"
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-900 leading-tight focus:outline-none focus:shadow-outline">{{ old('sinopsis', $buku->sinopsis) }}</textarea>
                        @error('sinopsis')
                            <div class="alert alert-danger text-red-500" role="alert">
                                {{ $message }}
                            </div>
                ...
                <div class="mb-4 flex flex-row flex-wrap">
                    <div class="w-full md:w-3/12 mb-4">
                        <img id="preview" src="{{ asset('images/' . $buku->foto) }}" alt="Foto Buku" class="w-80 h-80 rounded-lg border-2 border-white mt-4">
                    </div>
                    <div class="w-full md:w-9/12 mb-4">
                        <label for="foto" class="block text-gray-200 text-sm font-bold mb-2">Foto Buku:</label>
                        <input type="file" id="foto" name="foto" class="shadow appearance-none border rounded w-full py-2 px-3 text-black leading-tight focus:outline-none focus:shadow-outline bg-white" onchange="previewImage(this)" value="{{ $buku->foto }}">
                        @error('foto')
                            <div class="alert alert-danger text-red-500" role="alert">
                                {{ $message }}
                            </div>
